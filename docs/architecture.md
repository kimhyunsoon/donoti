...
- **카탈로그**: 쇼핑(inventory: emart·nintendo) / 영화(movie: cgv·lotte·megabox) / 시세(price: fx·stock). `backend/src/catalog.ts` ↔ `frontend/src/catalog.ts` 동기 유지. 브랜드 아이콘은 `frontend/public/brands/*.jpg`(공식 앱 아이콘 128px, iTunes Search API로 확보).
