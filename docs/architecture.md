...
| `notifications` | 알림 발송 큐. status(pending/sending/sent/failed) + attempts + next_attempt_at(epoch ms). 워커가 같은 회차의 **같은 source는 브라우저 알림 1개로 합쳐 발송**(4건 초과는 나눠서, 알림센터 행은 개별 유지) |
...
- **카탈로그**: 쇼핑(inventory: emart·nintendo·playstation) / 영화(movie: cgv·lotte·megabox) / 시세(price: fx·stock). `backend/src/catalog.ts` ↔ `frontend/src/catalog.ts` 동기 유지. 브랜드 아이콘은 `frontend/public/brands/*.jpg`(공식 앱 아이콘 128px, iTunes Search API로 확보).
...
- **이마트(emart) 크롤러**: `crawlers/emart.ts` — 이마트 앱 공유 링크 기반, 1알림 = 1상품 + N지점. 링크 해석: `emart.kr` 단축 → 302 Location의 `appLink.do?link=` 파싱. 지원 2유형: **digital**(디지털그랩 픽업, `panda/api/v1/digital-grab/item?skuCode&storeCode` + 취급지점 `digital-grab/store?skuCode`, **구매가능 판정은 `itemStatus === 'N'`만** — N=판매중, S/O=품절, E/U=판매종료, R=판매전, P=점포변경, J=구매완료. `buyLimit`의 maxBuyAvailableCount·personalBuyAvailableCount는 품절 지점에서도 양수일 수 있어 판정에 쓰면 오탐(스타필드 하남 사례)) / **product**(일반 매장 상품, `product/api/v1/info/1100/{sku}?storeId=`(POST {}) + 전지점 `search/api/v1/store/branch?storeType=E` + **지점별 재고 일괄** `store/stock/branch?storeType=E&skuCode=`). 오더픽·와인그랩 등은 등록 시 거부(앱 전용, 비로그인 500). **주의: digital storeCode(구로 3400)와 product storeId(은평 1033)는 다른 체계**. 알림은 구매가능 지점이 있으면 매회 발송(상태 무관), config `{type,sku,name,link,stores[],endOnStock}` — endOnStock이면 알림 후 soft delete. 등록 폼용 `POST /api/emart/resolve {url}`.
- **닌텐도(nintendo) 크롤러**: `crawlers/nintendo.ts` — 닌텐도 스토어(`store.nintendo.co.kr`) 상품 링크 붙여넣기 방식의 가격·할인 감시. 웹 스토어는 **Magento 기반**: 상품 페이지 HTML의 `"price_info":{...}` JSON(첫 등장 = 메인 상품 priceBox 설정)에서 `final_price`(판매가)·`regular_price`(정가) 파싱, 이름은 `<title>`. eShop 세일도 웹 스토어에 special price로 반영된다(할인 시 final < regular — 세일 상품 실측 검증: 정가 66,700 → 60,030). config `{productId(경로), name, link, onPriceChange}` — **onPriceChange(기본 true)는 직전 알림 가격과 다를 때만 발송, 1회차는 무조건 발송**(직전 가격은 `watch.state.lastPrice`에 기억, retry.ts의 failing 키와 공존). 본문: `23,500원 · 50% 할인\n정가 47,000원\n직전 알림 47,000원 → 23,500원`. 등록 폼용 `POST /api/nintendo/resolve {url}`.
- **플레이스테이션(playstation) 크롤러**: `crawlers/playstation.ts` — PS 스토어(`store.playstation.com`) `/concept/<id>`(대표 에디션 추적)·`/product/<id>`(해당 에디션 추적) 링크의 가격·할인 감시, 닌텐도와 동일 UX·config(`onPriceChange` 기본 true, `watch.state.lastPrice`·`lastPlusPrice` — 전용가 변동도 알림 트리거). 파싱: **Next.js SSR** — `__NEXT_DATA__` → `pageProps.batarangs.cta.text` 안의 `<script type="application/json">` JSON의 `cache`(apollo). concept이면 `Concept:<id>.defaultProduct.__ref`, product면 `Product:<id>` → `webctas[].__ref` → `price{basePriceValue=정가, discountedValue=판매가, endTime=할인 종료 epoch ms, serviceBranding}`. **일반가/PS Plus 전용가 구분은 serviceBranding(NONE/PS_PLUS)만 신뢰** — 전용가 CTA(UPSELL_PS_PLUS_DISCOUNT)가 목록 앞에 오고 isTiedToSubscription도 false라 순서·그 필드로 고르면 전용가가 일반가로 잡히는 함정(Lost in Play 실측: NONE 13,680 / PS_PLUS 12,540). 전용가는 본문 'PS Plus 전용가 N원' 줄로 표기. 할인 실측 검증: Split Fiction 59,000→38,350(-35%)+endTime. 상품명은 `name`에서 지원 언어 괄호 제거. 타국 링크도 ko-kr로 정규화. 할인 시 본문에 '~까지'(KST) 표기. 등록 폼용 `POST /api/playstation/resolve {url}`(plusPrice 포함). 프론트 폼은 닌텐도와 SHOP_PROVIDERS로 공유.
- **PS Plus 월간 무료 게임(playstation `mode:'plusMonthly'`)**: 링크 없이 등록하는 두 번째 감시 종류(폼의 '종류' 세그먼트). 소스: `playstation.com/ko-kr/ps-plus/whats-new/`의 `id="monthly-games"` 섹션 — 카드 마크업이 게임 카탈로그 섹션과 동일해 **앵커 뒤 첫 게임 링크부터 링크 간격 8,000자 초과 지점까지를 월간 클러스터로** 자른다(`/ko-kr/games/<slug>/` + 직전 h2/h3가 제목). 스토어 카테고리 API(WEB_EU_PSPLUS_MONTHLY_GAMES `038b4df3-…`)는 폐기되어 전 지역 빈 응답 — 쓰지 말 것. `onListChange`(기본 true)로 slug 목록(`state.lastGames`, 정렬·조인) 변동 시에만 알림, 1회차 무조건. 닌텐도는 월간 무료 게임 제도가 없어 해당 모드 없음.
